...
The formal IMS run is complete. The dataset loader produced 37,184 windows using 8192-point windows and 4096-point stride, with 18,592 normal windows and 18,592 fault-audit windows. The formal setting uses bearing-wise and condition-wise clients, CNN-AE reconstruction, 10 federated rounds, two local epochs, three random seeds, FedAvg, FedProx, FedBN, personalized variants, and client-specific, pooled, and adaptive calibration. IMS is much more difficult than CWRU-style strict splits: local-only remains the strongest reference, FedBN gives the best current federated operating point, personalized federated variants approach local-only under bearing-wise clients, and pooled calibration often increases miss rate. This should be reported as external validation evidence that federated deployment requires non-IID adaptation and client-aware calibration.
