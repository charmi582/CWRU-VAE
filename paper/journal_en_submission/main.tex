...
\title[A calibration-aware fuzzy health index for bearing anomaly detection]{A Calibration-Aware Fuzzy Health Index for Normal-Only Bearing Anomaly Detection: Strict Generalization Validation and External Paderborn Evaluation}

\author[1]{\fnm{Chia-Hsuan} \sur{Chang}}\email{s3B341027@gm.student.ncut.edu.tw}
\author[1]{\fnm{Chun-Lung} \sur{Chang}}\email{VincentChang0304@ncut.edu.tw}
\author*[1]{\fnm{Chuan-Kang} \sur{Liu}}\email{ckliu@ncut.edu.tw}

\affil[1]{\orgdiv{Department of Artificial Intelligence Application Engineering}, \orgname{Anonymous Institution}, \orgaddress{\country{Taiwan}}}

\abstract{Public bearing benchmarks have enabled rapid progress in data-driven condition monitoring, yet many results remain difficult to translate into deployable anomaly detection systems. A common reason is that experiments are often evaluated with window-level random splits, balanced fault distributions and ranking metrics such as \rocauc. Such settings can overstate practical performance when adjacent sliding windows or identical operating conditions appear in both training and testing. This paper presents a reproducible normal-only bearing anomaly detection workflow that treats threshold calibration and health-state interpretation as core components. The pipeline trains only on normal vibration windows, evaluates VAE, CNN-AE, LSTM-AE and Isolation Forest models under strict CWRU protocols, extends validation to the Paderborn bearing dataset, and converts calibrated anomaly scores into a fuzzy health index. On CWRU, reconstruction models retain high ranking performance under load-wise, fault-size-wise and file-wise splits, but false alarm behavior depends strongly on the threshold policy. On Paderborn, ranking remains informative while thresholded decisions become substantially harder, especially under conservative percentile or robust MAD thresholds. The results show that deployable bearing anomaly detection should report not only ranking metrics, but also calibration transfer, false alarm rate, miss rate, uncertainty rate, inference latency and model footprint. The main contribution is therefore a calibration-aware, reproducible and interpretable workflow for normal-only industrial health monitoring rather than another high-accuracy CWRU classifier.}

\keywords{bearing vibration; anomaly detection; normal-only learning; threshold calibration; fuzzy health index; condition monitoring}
...
The reproducible VAE-based bearing anomaly detection pipeline reported by Chang et al. \citep{tcse2026conference} provides useful context for the calibration-aware normal-only monitoring problem studied here.

Figure~\ref{fig:study_scope} summarizes the scope of the present study. The figure is intentionally placed before the contribution list because the central argument is not a single model block, but the connection between normal-only anomaly scoring, stricter validation, threshold behavior and interpretable health decisions.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.94\textwidth]{fig00_study_scope.png}
  \caption{Scope of the proposed calibration-aware normal-only bearing anomaly detection study.}
  \label{fig:study_scope}
\end{figure*}
...
\input{tables/tab_literature_positioning}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.96\textwidth]{fig01_research_pipeline.png}
  \caption{Two-layer view of the proposed calibration-aware normal-only bearing anomaly detection workflow. The upper layer defines the evaluation setting, while the lower layer separates AI anomaly scoring, threshold calibration, fuzzy health decision and deployment reporting.}
  \label{fig:pipeline}
\end{figure*}
...
\input{tables/tab_dataset_protocols}
...
\input{tables/tab_reproducibility}
...
\input{tables/tab_cwru_strict_results}
...
\input{tables/tab_paderborn_external_results}
...
\input{tables/tab_calibration_policies}
...
\subsection{Implications for condition-monitoring research}

The strongest positioning of this study is not model novelty, but evaluation maturity. The paper provides a reproducible workflow that connects strict splits, external Paderborn validation, threshold calibration, fuzzy health interpretation and deployment metrics. This is a credible contribution for studies at the intersection of applied artificial intelligence, mechanical condition monitoring and intelligent maintenance, provided that the work is framed as a calibration-aware workflow and evaluation study rather than a state-of-the-art architecture paper.

\section{Conclusion}
\label{sec:conclusion}

This paper presented a calibration-aware fuzzy health-index workflow for normal-only bearing anomaly detection. The evaluation includes strict CWRU protocols, an external Paderborn dataset, multiple baselines, threshold calibration policies and fuzzy health-state decisions. The results show that CWRU can remain highly separable under stricter splits, but thresholded behavior still depends on calibration. Paderborn external validation is substantially harder and exposes the practical importance of false alarm, miss rate and uncertainty reporting. The fuzzy health index provides an interpretable decision layer that makes warning and ambiguous regions visible.
...
Chia-Hsuan Chang contributed to conceptualization, implementation, experiments, analysis and manuscript drafting. Chun-Lung Chang contributed to supervision, methodology discussion and manuscript review. Chuan-Kang Liu contributed to supervision, research design, validation and manuscript review.
