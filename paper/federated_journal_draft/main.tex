...
\begin{itemize}
    ...
    \item We provide a deployment-oriented comparison among local-only, centralized, FedAvg, FedProx, and personalized federated training under bearing-wise and condition-wise client partitions.
...
\]
Only model parameters are transmitted. Raw vibration windows, validation scores, and local test windows remain at the client side unless explicitly used for aggregate reporting in the experiment. In addition to FedAvg, we evaluate FedProx, which adds a proximal regularization term during local optimization to reduce client drift under non-IID data. We also evaluate a personalized variant in which the federated global model is adapted locally for one additional epoch before client-side evaluation.
...
The main observation is that plain FedAvg does not automatically outperform local-only or centralized training. Under bearing-wise clients, FedAvg improves over local-only but remains below the centralized reference. FedProx alone is not consistently better than FedAvg, but the personalized FedProx variant provides the best current F1 operating point under bearing-wise clients. Under condition-wise clients, FedProx-personalized reaches a performance level comparable to the centralized reference. These results support a careful interpretation: federated learning enables privacy-preserving collaborative monitoring, but non-IID adaptation and client-aware calibration are necessary for practical deployment.
