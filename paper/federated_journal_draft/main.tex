...
Federated learning has received increasing attention in healthcare, manufacturing, Internet-of-Things, and predictive maintenance because it supports collaborative model training without centralizing raw data. FedAvg is the most widely used baseline: each client performs local optimization, and a server averages model parameters \citep{mcmahan2017fedavg}. For industrial monitoring, this privacy-preserving property is attractive, but non-IID client data can degrade convergence and make calibration difficult. FedProx addresses heterogeneous federated optimization by adding a proximal term to the local objective \citep{li2020fedprox}, while FedBN keeps BatchNorm statistics local to reduce feature-shift effects across clients \citep{li2021fedbn}. Recent federated fault-diagnosis studies further show the relevance of privacy-preserving distributed training for rotating machinery \citep{li2021modularflfault,survishno2023flbearing}. However, many such studies remain supervised and accuracy-centered. Therefore, a federated bearing-monitoring study should evaluate not only mean accuracy, but also client-level stability, calibration shift, and deployment cost.
