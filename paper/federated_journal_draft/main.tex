...
\begin{itemize}
    ...
    \item We provide a deployment-oriented comparison among local-only, centralized, and federated FedAvg training under bearing-wise and condition-wise client partitions.
...
\]
Only model parameters are transmitted. Raw vibration windows, validation scores, and local test windows remain at the client side unless explicitly used for aggregate reporting in the experiment.

\subsection{Calibration and Fuzzy Health Index}

Two calibration scopes are evaluated. In client-specific calibration, each client computes score anchors from its own validation-normal scores. In pooled calibration, validation-normal anchors are estimated from the union of validation scores across clients. For percentiles $p\in\{50,90,95,99\}$, the corresponding anchors are denoted as $q_p$.
...
\begin{itemize}
    ...
    \item \textbf{Federated}: clients train locally and share only model weights through FedAvg.
\end{itemize}

The main model is a CNN-based autoencoder. FedAvg is run for 10 rounds with two local epochs per round. The centralized reference is trained for 20 epochs. All formal experiments are repeated with seeds 42, 202, and 777. Fault windows are reserved for audit evaluation and are never used during training.
...
\input{tables/tab_federated_summary}

The main observation is that FedAvg does not automatically outperform local-only or centralized training. Under bearing-wise clients, centralized training with client-specific calibration obtains the highest F1 among the listed settings, while federated training remains below the centralized reference. Under condition-wise clients, FedAvg provides a comparable but still modest operating point. These results support a careful interpretation: federated learning enables privacy-preserving collaborative monitoring, but it does not remove the need for client-level calibration and stability analysis.
...
\input{tables/tab_communication_cost}
